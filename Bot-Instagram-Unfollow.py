...
class InstagramBot:
    ...
    def login(self):
        os.system('CLS')
        global cantidad
        cantidad = int(input(CYAN+ "Ingrese cantidad para dejar de seguir:"))
        os.system('CLS')
        sleep(0.6)
        print(RED+ "OK!")
        sleep(0.6)
    ...
    def follow_user(self, user):
        self.nav_user(user)
        try:
            follow_button = self.driver.find_elements_by_xpath("/html/body/div[1]/section/main/div/header/section/div[1]/div[1]/div/div[2]/div/span/span[1]/button")[0]
            follow_button.click()
            sleep(0.5)
            confirmar_boton = self.driver.find_element_by_xpath("/html/body/div[6]/div/div/div/div[3]/button[1]")
    ...
    def user_followers(self, user, limit=70):
        self.nav_user(user)
        sleep(self.random_number_generator(2, 4))
        followers_btn = self.driver.find_element_by_xpath(f"//a[contains(@href, '/{user}/following/')]")
        followers_btn.click()
        sleep(self.random_number_generator(2, 4))
        fBody = self.driver.find_element_by_xpath("//div[@class='isgrP']")
        scroll = 0
        while scroll < 50:
            print(WHITE+ "Obteniendo Seguidores...")
        ...
        os.system('CLS')
        print(YELLOW+"Se dejaran de seguir las siguientes cuentas:")
    ...
    def follow_user_followers(self, user, limit=50):
        users = self.user_followers(user, limit)
...
if __name__ == "__main__":
 ...
 bot.follow_user_followers("account")